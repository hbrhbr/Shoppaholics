...
class _CheckoutState extends State<Checkout> {
  ...
  fetchList() async {
    ...
    bool isMYFatorahAvailable = false;
    bool isKNETAvaiable = false;
    paymentTypeResponseList.forEach((method) {
      if (method.name.toLowerCase() == "knet") {
        isKNETAvaiable = true;
      }
      if (method.name.toLowerCase() == "myfatoorah") {
        isMYFatorahAvailable = true;
      }
    });
    if (!isMYFatorahAvailable) {
      paymentTypeResponseList.add(
        PaymentTypeResponse(
          title: "Checkout with MyFatoorah",
          name: "MyFatoorah",
          payment_type: "my_fatoorah",
          payment_type_key: "my_fatoorah",
          image: "assets/my_fatoorah.png",
          isOnline: false,
        ),
      );
    }
    if (!isKNETAvaiable) {
      paymentTypeResponseList.add(
        PaymentTypeResponse(
          title: "Checkout with KNET",
          name: "KNET",
          payment_type: "knet",
          payment_type_key: "knet",
  ...
  onPressPlaceOrderOrProceed() {
    ...
    } else if (_selected_payment_method == "knet") {
      if (_grandTotalValue == 0.00) {
        ToastComponent.showDialog("Nothing to pay", context,
            gravity: Toast.CENTER, duration: Toast.LENGTH_LONG);
        return;
              ...
              Container(
                  ...
                  child: Padding(
                      ...
                      child:
                          ...
                          _paymentTypeList[index].isOnline ?? false
                              ...
                              : Image.asset(_paymentTypeList[index].image))),
              Container(
                width: 150,
                child: Column(
