...
class PaymentRepository {
  ...
  Future<Map<String, dynamic>> getMyFatoorahCredentails() async {
    final response = await http.get(
      "${AppConfig.BASE_URL}/fatoorah/credintials",
    );
    print(response.body.toString());
  ...
  Future<FlutterwaveUrlResponse> getFlutterwaveUrlResponse(
    ...
    var url =
        "${AppConfig.BASE_URL}/flutterwave/payment/url?payment_type=${payment_type}&combined_order_id=${combined_order_id}&amount=${amount}&user_id=${user_id.value}";
    //print(url);
  ...
  Future<OrderCreateResponse> getOrderCreateResponseFromManualPayment(
    ...
    final response =
        await http.post("${AppConfig.BASE_URL}/payments/pay/manual",
            headers: {
              "Content-Type": "application/json",
              "Authorization": "Bearer ${access_token.value}"
            },
  ...
  Future<BkashBeginResponse> getBkashBeginResponse(
      ...
      @required double amount) async {
    final response = await http.get(
      "${AppConfig.BASE_URL}/bkash/begin?payment_type=${payment_type}&combined_order_id=${combined_order_id}&amount=${amount}&user_id=${user_id.value}",
  ...
  Future<BkashPaymentProcessResponse> getBkashPaymentProcessResponse(
    ...
    final response = await http.post("${AppConfig.BASE_URL}/bkash/api/process",
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer ${access_token.value}"
        },
  ...
  Future<SslcommerzBeginResponse> getSslcommerzBeginResponse(
      ...
      @required double amount) async {
    final response = await http.get(
      "${AppConfig.BASE_URL}/sslcommerz/begin?payment_type=${payment_type}&combined_order_id=${combined_order_id}&amount=${amount}&user_id=${user_id.value}",
      headers: {"Authorization": "Bearer ${access_token.value}"},
    );
  ...
  Future<NagadBeginResponse> getNagadBeginResponse(
      ...
      @required double amount) async {
    final response = await http.get(
      "${AppConfig.BASE_URL}/nagad/begin?payment_type=${payment_type}&combined_order_id=${combined_order_id}&amount=${amount}&user_id=${user_id.value}",
      headers: {"Authorization": "Bearer ${access_token.value}"},
    );
